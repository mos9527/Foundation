...
template<> asset_handle AssetRegistry::load<StaticMeshAsset>(mesh_static& mesh) {
...
template<> void AssetRegistry::upload<StaticMeshAsset>(RHI::Device* device, StaticMeshAsset* asset) {		
	...
	asset->vertexBuffer = std::make_unique<RHI::Buffer>(device, create_buffer_desc(asset->vertexInitialData));
	asset->numVertices = asset->vertexInitialData.size();
