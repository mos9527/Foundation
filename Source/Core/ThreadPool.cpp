...
namespace Foundation::Core
{
    ThreadPool::ThreadPool(size_t numThreads, size_t maxTasks, Allocator* alloc, StringView name) :
        mAllocator(alloc), mName(name),
        mJobs{JobQueue(maxTasks, alloc), JobQueue(maxTasks, alloc), JobQueue(maxTasks, alloc)}, mThreads(alloc)
    {
    ...
    void ThreadPool::ThreadPoolWorker(size_t id)
    {
        ...
        while (!mShutdown)
        {
            ...
            // WaitOnAddress
            // [https://github.com/microsoft/STL/blob/43e96b29a971a7a26aceaa539d22cbedfcf13687/stl/src/atomic_wait.cpp#L99]
