...
void Geometry::load_static_mesh(Renderer* renderer, mesh_static const& mesh) {
	...
	intermediateBuffer = renderer->get_device()->AllocateIntermediateBuffer(desc);
	...
	auto upload = [&](auto data) {
		...
		intermediateBuffer->Update(data.data(), size, offset);
		return size;
	};
	// Vertices
...
entt::entity Geometry::upload(Renderer* renderer, RHI::CommandList* cmdList) {
	...
	// Schedule a copy
	renderer->get_buffer(geometryBuffer).QueueCopy(cmdList, intermediateBuffer.get(), 0, 0, allocSize);
	// Release the intermediate buffer ref. Actual destruction / GC happens in RHI::Device::FlushIntermediateBuffers
