...
namespace Foundation::Async
{
    ...
    template<typename Lambda, typename ReturnType> class ThreadPoolLambdaJob final : public ThreadPoolJob
    {
        Lambda m_func;
        SharedPromise<ReturnType> m_promise;
    public:
        ThreadPoolLambdaJob(SharedPromise<ReturnType> promise, Lambda&& func) :
            m_func(func), m_promise(promise)
        {}
    ...
    public:
         ...
         */
        template <typename Lambda, typename... Args> auto PushPriority(Lambda&& func, int priority, Args&&... args)
        {
            CHECK(!m_shutdown);
            ...
            auto job = ConstructUniqueBase<ThreadPoolJob, LambdaType>(
                m_allocator, promise, std::forward<PackagedType>(packaged)
            );
            {
         ...
         * @return @ref SharedPromise that will be set when the job is completed.
         */
        template<typename Lambda, typename... Args> auto Push(Lambda&& func, Args&&... args)
        {
