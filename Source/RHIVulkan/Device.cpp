...
Allocator* VulkanDevice::GetAllocator() const { return m_app.GetAllocator(); }
vk::AllocationCallbacks const& VulkanDevice::GetVkAllocatorCallbacks() const { return m_app.GetVkAllocatorCallbacks(); }
    ...
    m_swapchain_present_modes(GetAllocator()), m_storage(GetAllocator(), kDeviceStorageReserveSize), window_(window)
{
    ...
    if (window)
    {
        ...
        if (glfwCreateWindowSurface(*m_app.GetVkInstance(), static_cast<GLFWwindow*>(window->GetNative()), nullptr,
                                    &surface) != VK_SUCCESS)
    ...
    for (auto i : unique_queues)
    {
        ...
        if (i == graphics)
            m_queues->graphics = handle;
        if (i == compute)
            m_queues->compute = handle;
        if (i == transfer)
            m_queues->transfer = handle;
        if (i == present)
            m_queues->present = handle;
    }
    // Initialize VMA
    ...
    CHECK(vmaCreateAllocator(&allocator_info, &m_vkAllocator) == VK_SUCCESS &&
...
RHIDeviceQueue* VulkanDevice::GetDeviceQueue(RHIDeviceQueueType type) const
{
    ...
    case RHIDeviceQueueType::Present:
        return m_queues->Get(m_queues->present);
    case RHIDeviceQueueType::Compute:
        return m_queues->Get(m_queues->compute);
    case RHIDeviceQueueType::Transfer:
        return m_queues->Get(m_queues->transfer);
    case RHIDeviceQueueType::Graphics:
        return m_queues->Get(m_queues->graphics);
    default:
        break;
    }
    return nullptr;
...
Span<RHIResourceFormat const> VulkanDevice::GetSwapchainSupportedFormats() const { return m_swapchain_formats; }
...
RHISwapchain* VulkanDevice::GetSwapchain(Handle handle) const { return m_storage.GetObjectPtr<RHISwapchain>(handle); };
void VulkanDevice::DestroySwapchain(Handle handle) { m_storage.DestroyObject(handle); }
...
void VulkanDevice::DestroyPipelineState(Handle handle) { m_storage.DestroyObject(handle); }
...
void VulkanDevice::DestroyShaderModule(Handle handle) { m_storage.DestroyObject(handle); }
...
void VulkanDevice::DestroyCommandPool(Handle handle) { m_storage.DestroyObject(handle); }
...
void VulkanDeviceSemaphore::DebugSetObjectName(const char* name)
{
    ...
    m_device.GetVkDevice().setDebugUtilsObjectNameEXT({.objectType = vk::ObjectType::eSemaphore,
...
void VulkanDeviceFence::DebugSetObjectName(const char* name)
{
    ...
    m_device.GetVkDevice().setDebugUtilsObjectNameEXT({.objectType = vk::ObjectType::eFence,
                                                       .objectHandle = reinterpret_cast<uint64_t>(handle),
                                                       .pObjectName = name});
}
...
void VulkanDevice::DestroySemaphore(Handle handle) { m_storage.DestroyObject(handle); }
...
RHIDeviceFence* VulkanDevice::GetFence(Handle handle) const { return m_storage.GetObjectPtr<RHIDeviceFence>(handle); }
void VulkanDevice::DestroyFence(Handle handle) { m_storage.DestroyObject(handle); }
    ...
    Span<const Pair<RHIDeviceObjectHandle<RHIDeviceSemaphore>, size_t>> semaphores, size_t timeout)
{
    ...
    auto res =
        m_device.waitSemaphores(vk::SemaphoreWaitInfo{.semaphoreCount = static_cast<uint32_t>(vk_semaphores.size()),
...
void VulkanDevice::DebugSetObjectName(const char* name)
{
    ...
    m_device.setDebugUtilsObjectNameEXT({.objectType = vk::ObjectType::eDevice,
                                         .objectHandle = reinterpret_cast<uint64_t>(handle),
                                         .pObjectName = name});
}
...
void VulkanDeviceQueue::Submit(SubmitDesc const& desc) const
{
    ...
    CHECK_MSG(desc.waits_stages.size() == swaits.size(),
              "Number of wait stages ({}) must match number of wait semaphores ({})", desc.waits_stages.size(),
              swaits.size());
...
void VulkanDeviceQueue::DebugSetObjectName(const char* name)
{
    ...
    m_device.GetVkDevice().setDebugUtilsObjectNameEXT({.objectType = vk::ObjectType::eQueue,
                                                       .objectHandle = reinterpret_cast<uint64_t>(handle),
                                                       .pObjectName = name});
}
...
RHIBuffer* VulkanDevice::GetBuffer(Handle handle) const { return m_storage.GetObjectPtr<RHIBuffer>(handle); }
void VulkanDevice::DestroyBuffer(Handle handle) { m_storage.DestroyObject(handle); }
...
RHITexture* VulkanDevice::GetImage(Handle handle) const { return m_storage.GetObjectPtr<RHITexture>(handle); }
void VulkanDevice::DestroyImage(Handle handle) { m_storage.DestroyObject(handle); }

void VulkanDeviceDescriptorSetLayout::DebugSetObjectName(const char* name)
{
    VkDescriptorSetLayout handle = *m_layout;
    m_device.GetVkDevice().setDebugUtilsObjectNameEXT({.objectType = vk::ObjectType::eDescriptorSetLayout,
                                                       .objectHandle = reinterpret_cast<uint64_t>(handle),
                                                       .pObjectName = name});
}
...
void VulkanDevice::DestroyDescriptorSetLayout(Handle handle) { m_storage.DestroyObject(handle); }
...
void VulkanDevice::DestroyDescriptorPool(Handle handle) { m_storage.DestroyObject(handle); }


void VulkanDeviceSampler::DebugSetObjectName(const char* name)
{
    VkSampler handle = *m_sampler;
    m_device.GetVkDevice().setDebugUtilsObjectNameEXT({.objectType = vk::ObjectType::eSampler,
                                                       .objectHandle = reinterpret_cast<uint64_t>(handle),
                                                       .pObjectName = name});
}
    ...
    RHIDeviceSampler(device, desc), m_device(device)
{
    ...
    m_sampler = vk::raii::Sampler(
        m_device.GetVkDevice(),
        vk::SamplerCreateInfo{.magFilter = vkFilterFromRHISamplerFilter(desc.filter.mag_filter),
                              .minFilter = vkFilterFromRHISamplerFilter(desc.filter.min_filter),
                              .mipmapMode = desc.mipmap.mipmap_mode == SamplerDesc::Mipmap::Linear
...
void VulkanDevice::DestroySampler(Handle handle) { m_storage.DestroyObject(handle); }
