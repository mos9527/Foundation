...
#pragma region PSO
         ...
         */
        virtual RHICommandList& BindDescriptorSet(RHIDevicePipelineType bindpoint, RHIPipelineState* pipeline,
                                                  Span<RHIDeviceDescriptorSet* const> sets, size_t first = 0,
                                                  Span<const uint32_t> dynamicOffsets = {}) = 0;
#pragma endregion
#pragma region Rasterizer
        virtual RHICommandList& SetViewport(float x, float y, float width, float height, float depth_min = 0.0,
                                            float depth_max = 1.0, bool flipY = false) = 0;
        virtual RHICommandList& SetScissor(uint32_t x, uint32_t y, uint32_t width, uint32_t height) = 0;
        virtual RHICommandList& Draw(uint32_t vertex_count, uint32_t instance_count = 1, uint32_t first_vertex = 0,
                                     uint32_t first_instance = 0) = 0;
        virtual RHICommandList& DrawIndexed(uint32_t index_count, uint32_t instance_count = 1, uint32_t first_index = 0,
                                            int32_t vertex_offset = 0, uint32_t first_instance = 0) = 0;
        virtual RHICommandList& DrawIndexedIndirectCount(RHIBuffer* cmd_buffer, size_t cmd_offset,
                                                         RHIBuffer* count_buffer, size_t count_offset,
                                                         uint32_t max_draw_count, uint32_t cmd_stride) = 0;
        virtual RHICommandList& DrawMeshTasks(uint32_t group_count_x, uint32_t group_count_y,
...
#pragma region Transfer Queue
        ...
        virtual RHICommandList& FillBuffer(RHIBuffer* buffer, uint32_t value, size_t offset = 0,
                                           size_t size = kFullSize) = 0;
        virtual RHICommandList& CopyBuffer(RHIBuffer* src_buffer, RHIBuffer* dst_buffer,
                                           Span<const CopyBufferRegion> regions) = 0;
        ...
        virtual RHICommandList& CopyImage(RHITexture* src_image, RHITextureLayout src_layout, RHITexture* dst_image,
...
#pragma region Graphics Pipeline
        ...
        virtual RHICommandList& BindVertexBuffer(uint32_t index, Span<RHIBuffer* const> buffers,
                                                 Span<const size_t> offsets) = 0;
        virtual RHICommandList& BindIndexBuffer(RHIBuffer* buffer, size_t offset = 0,
                                                RHIResourceFormat format = RHIResourceFormat::R32Uint) = 0;
...
#pragma endregion
    ...
    {
        static RHICommandList* Get(RHICommandPool const* pool, Handle handle) { return pool->GetCommandList(handle); }
        static void Destroy(RHICommandPool* pool, Handle handle) { pool->DestroyCommandList(handle); }
    };
} // namespace Foundation::RHI
