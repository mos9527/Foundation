...
namespace Foundation::RHI
{
    ...
    public:
        struct SamplerDesc
        {
            ...
            struct AddressMode
            {
                enum Mode
                {
                    ...
                    v{Repeat}, w{Repeat}; // Address modes for U, V, W coordinates
            } addressMode;
            struct Mipmap
            {
                enum MipmapMode
            ...
            struct Filter
            {
                enum Type
                {
                    ...
                    magFilter{Linear}; // Minification and magnification filters
            } filter;
            struct LOD
            {
        ...
        RHIDeviceSampler(RHIDevice const& device, SamplerDesc const& desc) : mDevice(device), mDesc(desc) {}
    ...
    public:
        ...
        RHIDeviceQueryPool(RHIDevice const& device, QueryPoolDesc const& desc) : mDevice(device), mDesc(desc) {}
    ...
    public:
        ...
        [[nodiscard]] virtual RHIPipelineStateCache* GetPipelineCache(Handle handle) const = 0;
        virtual void DestroyPipelineCache(Handle handle) = 0;

        [[nodiscard]] virtual RHIDeviceScopedHandle<RHIPipelineState>
        CreatePipelineState(RHIPipelineState::PipelineStateDesc const& desc) = 0;
        ...
        [[nodiscard]] virtual RHIDeviceScopedHandle<RHIShaderModule>
        CreateShaderModule(RHIShaderModule::ShaderModuleDesc const& desc) = 0;
        [[nodiscard]] virtual RHIShaderModule* GetShaderModule(Handle handle) const = 0;
        virtual void DestroyShaderModule(Handle handle) = 0;

        [[nodiscard]] virtual RHIDeviceScopedHandle<RHICommandPool>
        CreateCommandPool(RHICommandPool::PoolDesc type) = 0;
        ...
        [[nodiscard]] virtual RHIDeviceScopedHandle<RHIDeviceSemaphore> CreateSemaphore(bool is_timeline = false) = 0;
        [[nodiscard]] virtual RHIDeviceSemaphore* GetSemaphore(Handle handle) const = 0;
        virtual void DestroySemaphore(Handle handle) = 0;

        [[nodiscard]] virtual RHIDeviceScopedHandle<RHIDeviceFence> CreateFence(bool signaled = true) = 0;
        [[nodiscard]] virtual RHIDeviceFence* GetFence(Handle handle) const = 0;
        virtual void DestroyFence(Handle handle) = 0;

        [[nodiscard]] virtual RHIDeviceScopedHandle<RHIBuffer> CreateBuffer(RHIBufferDesc const& desc) = 0;
        [[nodiscard]] virtual RHIBuffer* GetBuffer(Handle handle) const = 0;
        virtual void DestroyBuffer(Handle handle) = 0;

        [[nodiscard]] virtual RHIDeviceScopedHandle<RHITexture> CreateTexture(RHITextureDesc const& desc) = 0;
        ...
        [[nodiscard]] virtual RHIDeviceScopedHandle<RHIDeviceDescriptorSetLayout>
        CreateDescriptorSetLayout(RHIDeviceDescriptorSetLayoutDesc const& desc) = 0;
        ...
        [[nodiscard]] virtual RHIDeviceScopedHandle<RHIDeviceDescriptorPool>
        CreateDescriptorPool(RHIDeviceDescriptorPool::PoolDesc const& desc) = 0;
        ...
        [[nodiscard]] virtual RHIDeviceScopedHandle<RHIDeviceSampler>
        CreateSampler(RHIDeviceSampler::SamplerDesc const& desc) = 0;
        [[nodiscard]] virtual RHIDeviceSampler* GetSampler(Handle handle) const = 0;
        virtual void DestroySampler(Handle handle) = 0;

        [[nodiscard]] virtual RHIDeviceScopedHandle<RHIDeviceQueryPool>
         ...
         * @param timeout Wait timeout in nanoseconds. Set to 0 for no wait and return immediately, -1 for infinite
         * wait.
         ...
         */
        virtual bool WaitForTimelineSemaphores(Span<const Pair<RHIDeviceSemaphore*, size_t>> semaphores,
                                               size_t timeout) = 0;
    ...
    {
        static RHISwapchain* Get(RHIDevice const* device, Handle handle) { return device->GetSwapchain(handle); }
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroySwapchain(handle); }
    };
    template <>
    struct RHIObjectTraits<RHIDevice, RHIPipelineState>
    {
        ...
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroyPipelineState(handle); }
    };
    template <>
    struct RHIObjectTraits<RHIDevice, RHIShaderModule>
    {
        static RHIShaderModule* Get(RHIDevice const* device, Handle handle) { return device->GetShaderModule(handle); }
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroyShaderModule(handle); }
    };
    template <>
    struct RHIObjectTraits<RHIDevice, RHICommandPool>
    {
        static RHICommandPool* Get(RHIDevice const* device, Handle handle) { return device->GetCommandPool(handle); }
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroyCommandPool(handle); }
    };
    template <>
    struct RHIObjectTraits<RHIDevice, RHIDeviceSemaphore>
    {
        static RHIDeviceSemaphore* Get(RHIDevice const* device, Handle handle) { return device->GetSemaphore(handle); }
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroySemaphore(handle); }
    };
    template <>
    struct RHIObjectTraits<RHIDevice, RHIDeviceFence>
    {
        static RHIDeviceFence* Get(RHIDevice const* device, Handle handle) { return device->GetFence(handle); }
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroyFence(handle); }
    };
    template <>
    struct RHIObjectTraits<RHIDevice, RHIBuffer>
    {
        static RHIBuffer* Get(RHIDevice const* device, Handle handle) { return device->GetBuffer(handle); }
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroyBuffer(handle); }
    };
    template <>
    struct RHIObjectTraits<RHIDevice, RHITexture>
    {
        static RHITexture* Get(RHIDevice const* device, Handle handle) { return device->GetTexture(handle); }
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroyTexture(handle); }
    };
    template <>
    struct RHIObjectTraits<RHIDevice, RHIDeviceDescriptorSetLayout>
    {
        ...
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroyDescriptorSetLayout(handle); }
    };
    template <>
    struct RHIObjectTraits<RHIDevice, RHIDeviceDescriptorPool>
    {
        ...
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroyDescriptorPool(handle); }
    };
    template <>
    struct RHIObjectTraits<RHIDevice, RHIDeviceSampler>
    {
        static RHIDeviceSampler* Get(RHIDevice const* device, Handle handle) { return device->GetSampler(handle); }
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroySampler(handle); }
    };
    template <>
    ...
    {
        static RHIDeviceQueryPool* Get(RHIDevice const* device, Handle handle) { return device->GetQueryPool(handle); }
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroyQueryPool(handle); }
    };
    template <>
    struct RHIObjectTraits<RHIDevice, RHIAccelerationStructure>
    {
        ...
        {
            return device->GetAccelerationStructure(handle);
        }
        static void Destroy(RHIDevice* device, Handle handle) { device->DestroyAccelerationStructure(handle); }
    };
} // namespace Foundation::RHI
