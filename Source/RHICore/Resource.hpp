...
namespace Foundation::RHI
{
    ...
    public:
        ...
        RHIBuffer(RHIDevice const& device, RHIBufferDesc const& desc) : mDevice(device), mDesc(desc) {}
        /**
        ...
        [[nodiscard]] virtual RHIBufferScopedHandle<RHIBuffer> CreateAliasedBuffer(RHIBufferDesc const& desc,
                                                                                   size_t offset = 0) = 0;
    ...
    struct RHITextureSubresourceRange
    {
         ...
         */
        static RHITextureSubresourceRange Create(RHITextureAspectFlag aspect = RHITextureAspectFlagBits::Color,
        ...
        [[nodiscard]] constexpr bool IsValid() const { return layer.aspect.value && mipCount && layer.layerCount; }
    };
    ...
    public:
        ...
        RHITexture(RHIDevice const& device, RHITextureDesc const& desc) : mDevice(device), mDesc(desc) {}

        [[nodiscard]] virtual RHITextureScopedHandle<RHITextureView>
        CreateTextureView(RHITextureViewDesc const& desc) = 0;
        [[nodiscard]] virtual RHITextureView* GetImageView(Handle handle) const = 0;
        virtual void DestroyImageView(Handle handle) = 0;

        [[nodiscard]] virtual RHITextureScopedHandle<RHITexture> CreateAliasedTexture(RHITextureDesc const& desc,
                                                                                      size_t offset = 0) = 0;
    ...
    public:
        RHITextureView(RHITexture const& image, RHITextureViewDesc const& desc) : mImage(image), mDesc(desc) {}
    ...
    {
        static RHITextureView* Get(RHITexture const* image, Handle handle) { return image->GetImageView(handle); }
        static void Destroy(RHITexture* image, Handle handle) { image->DestroyImageView(handle); }
    };
    template <>
    struct RHIObjectTraits<RHIBuffer, RHIBuffer>
    {
        static RHIBuffer* Get(RHIBuffer const* buffer, Handle handle) { return buffer->GetAliasedBuffer(handle); }
        static void Destroy(RHIBuffer* buffer, Handle handle) { buffer->DestroyAliasedBuffer(handle); }
    };
    template <>
    struct RHIObjectTraits<RHITexture, RHITexture>
    {
        static RHITexture* Get(RHITexture const* texture, Handle handle) { return texture->GetAliasedTexture(handle); }
        static void Destroy(RHITexture* texture, Handle handle) { texture->DestroyAliasedTexture(handle); }
    };
} // namespace Foundation::RHI
