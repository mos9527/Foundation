...
void FRunningEnter()
{
    ...
    if (GRendererMode == ERendererMode::PathTracer)
    {
        PTReadbackHandles ptReadback;
        PathTracerSetup(GContext, GRendererConfig, scene, ptReadback);
        GPickResultBuffer = GContext->renderer->DerefResource(ptReadback.pickResultBuffer).Get<RHIBuffer*>();
    }
    else
    {
