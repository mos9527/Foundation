...
{
    MeshScratchBuffers sceneMeshDataFromVertexIndex(Span<MeshVertex> vertices, Span<MeshIndex> indices, Allocator* allocator,
                                           int numLods, const bool buildMeshlets)
    {
        ...
        MeshScratchBuffers res(allocator);
    ...
    {
        Params params{};
        mSceneParamsAllocation = scene->PushShared(Span<Params>(params).AsBytes(), alignof(Params));
    }
    void Scene::CommitParams()
    {
        Params params{
            .camera = mCamera.GetParams(),
            .cullParams = mCullingCamera.GetCullParams(),
            .instanceCount = mInstanceCount // TODO. Again...
        };
        mGPUScene->UpdateShared(mSceneParamsAllocation,Span<Params>(params).AsBytes());
    }
    ...
    SceneHandle Scene::PushMesh(MeshScratchBuffers const& data)
    {
        ...
        alloc.self = mGPUScene->PushConst(
            Span<const MeshAllocation>(alloc).AsBytes(), alignof(MeshAllocation)
        );
    ...
    Span<Instance> Scene::MapInstances() {
        return mGPUScene->MapInstanceData<Instance>();
    }
