...
MeshVertexCompact MeshVertexCompact::Pack(const vec3 pos, const vec3 normal, const vec3 tangent, const vec3 bitangent, const vec2 uv)
{
    MeshVertexCompact v;
    ...
    v.tp = {
        .tx = quantizeSnormShifted(tangentOct.x, 8),
        .ty = quantizeSnormShifted(tangentOct.y, 8)
    };
    vec2 normalOct = packUnitOctahedral(normal);
    bool sgn = dot(cross(normal, tangent), bitangent) < 0;
    v.np = {
        .nx = quantizeSnormShifted(normalOct.x, 15),
        .ny = quantizeSnormShifted(normalOct.y, 15),
        .sign = sgn
    };
...
void MeshVertexCompact::Unpack(vec3& pos, vec3& normal, vec3& tangent, vec3& bitangent, vec2& uv) const
{
    ...
    vec2 tangentOct = {
        dequantizeSnormShifted(tp.tx, 8),
        dequantizeSnormShifted(tp.ty, 8)
    };
    tangent = unpackUnitOctahedral(tangentOct);
    vec2 normalOct = {
        dequantizeSnormShifted(np.nx, 15),
        dequantizeSnormShifted(np.ny, 15)
    };
...
{
    return MeshoptAllocator.Allocate(size, alignof(std::max_align_t));
}
static void meshoptFree(void* ptr)
{
    MeshoptAllocator.Deallocate(ptr);
}
                                    ...
                                    const Vector<MeshIndex>& indices)
{
    ...
    outMeshletVertices.resize(indices.size()), outMeshletTriangles.resize(indices.size());
    ...
    size_t meshlets = meshopt_buildMeshlets(
        reinterpret_cast<meshopt_Meshlet*>(outMeshlet.data()),
        outMeshletVertices.data(),
        outMeshletTriangles.data(),
        indices.data(),
        indices.size(),
        &vertices[0].pos.x,
        vertices.size(),
